...
class SimplexTable:
    def __init__(self, system: Optional[RestrictionSystem] = None):
        ...
        """
        self.__rows__: List[BaseRow] = []
        self.__objective_function__: BaseRow = BaseRow()
        self.__basis__: List[Optional[int]] = []
        if system != None:
    ...
    def get_solution(self):
        ...
        for i in range(len(self.__objective_function__.coefficients)):
            if self.__basis__.__contains__(i):
                row_index = self.__basis__.index(i)
                result.coefficients.append(
                    self.__rows__[row_index].free_member)
            else:
                result.coefficients.append(0)
        result.free_member = self.__objective_function__.free_member
        return result
